...
import { updatePaymentStatusSchema } from '../validators/payment.schema';
...
router.patch('/:id/verify', authenticate, requireRole('admin'), validateBody(updatePaymentStatusSchema), (req, res) => 
  paymentController.verify(req, res)
);

// PATCH /payments/:id/reject - Rechazar pago (solo admin)
router.patch('/:id/reject', authenticate, requireRole('admin'), validateBody(updatePaymentStatusSchema), (req, res) => 
